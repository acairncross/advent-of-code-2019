...
let rec eval program pc input =
  let open Option in
  let* instruction_header = parse_header program.(pc) in
  match instruction_header with
    | Add(m1, m2, _) ->
       evalBinop program pc (+) (m1, m2) input
    | Mul(m1, m2, _) ->
       evalBinop program pc ( * ) (m1, m2) input
    | Load(_) ->
       program.(program.(pc+1)) <- input;
       eval program (pc+2) input
    | Output(m) ->
       (* Out_channel.printf "Output: %d\n" (read_param_value program m program.(pc+1)); *)
       let* result = eval program (pc+2) input in
       return (read_param_value program m program.(pc+1) :: result)
    | JumpIfTrue(m1, m2) ->
       if read_param_value program m1 program.(pc+1) = 0
       then eval program (pc+3) input
       else eval program (read_param_value program m2 program.(pc+2)) input
    | JumpIfFalse(m1, m2) ->
       if read_param_value program m1 program.(pc+1) = 0
       then eval program (read_param_value program m2 program.(pc+2)) input
       else eval program (pc+3) input
    | LessThan(m1, m2, _) ->
       evalBinop program pc (fun x y -> if x < y then 1 else 0) (m1, m2) input
    | Equals(m1, m2, _) ->
       evalBinop program pc (fun x y -> if x = y then 1 else 0) (m1, m2) input
    | Halt ->
       return []

and evalBinop program pc op (m1, m2) input =
  ...
  eval program (pc+4) input

let eval0 program phase input =
  let open Option in
  let* instruction_header = parse_header program.(0) in
  match instruction_header with
    | Load(_) ->
       program.(program.(1)) <- phase;
       eval program 2 input
    | _ -> None

let eval_chain program phases input =
  List.fold_right
    ~f:(fun phase inputOpt ->
      let* input = inputOpt in
      let* output = eval0 (Array.copy program) phase input in
      match output with
      | [output] -> Some(output)
      | _ -> None)
    ~init:(Some(input))
...
let rec permutations = function
  ...
  | xs ->
     List.join
       (List.map
          ~f:(fun x ->
            List.map ~f:(fun perm -> x :: perm) (permutations (remove x xs)))
          xs)

let rec cat_options = function
  | [] -> []
  | None :: tl -> cat_options tl
  | Some(x) :: tl -> x :: cat_options tl
...
let _ =
  let program =
    ...
    |> Array.of_list in
  let phase_sequences = permutations [0;1;2;3;4] in
  let* max_signal =
    phase_sequences
    |> List.map ~f:(fun phases -> eval_chain program phases 0)
    |> cat_options
    |> List.max_elt ~compare:(Int.compare) in
  Some(Out_channel.printf "%d\n" max_signal)
