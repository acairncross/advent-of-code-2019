...
let turn turn_dir cur_dir =
  match turn_dir with
  | 0 -> turn_left cur_dir
  | 1 -> turn_right cur_dir
  | _ -> failwith "unexpected turn direction"

let paint map (x, y) color =
  Map.update map (x, y) ~f:(fun v ->
      match v with
      | Some(_) -> color
      | None -> color)
...
let rec eval_with_robot program pc rb panels ((x, y), dir) =
  let open Option in
  let cur_color = find_with_default panels (x, y) 0 in
  let* result = expanding_eval program pc rb [cur_color] in
  match result with
  | Paused([color'; turn_dir], (program', pc', rb')) ->
     let panels' = paint panels (x, y) color' in
     let dir' = turn turn_dir dir in
     let (i, j) = dir' in
     let pos' = (x+i, y+j) in
     eval_with_robot program' pc' rb' panels' (pos', dir')
  | Paused(_, _) -> failwith "program paused with unexpected number of outputs"
  | Finished(_) -> return panels
  (* | Finished(_) -> failwith "terminated with unexpected output" *)
...
let _ =
  ...
  match eval_with_robot program 0 0 (Map.singleton (module Int_pair_comparator) (0, 0) 1) ((0, 0), (0, 1)) with
  | Some(map) -> print_map map
  | None -> failwith "the program failed"
